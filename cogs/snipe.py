...
class Snipe(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if isinstance(error, RequiresSnipe):
            return await ctx.send(
                "Seems this guild isn't configured for snipes. It is an opt-in basis.\nHave a moderator/admin run `snipe setup`."
            )
    ...
    async def _gen_edit_embeds(
        self, records: typing.List[typing.Dict[str, typing.Any]]
    ) -> typing.List[discord.Embed]:
        embeds = []
        for record in records:
            channel = self.bot.get_channel(record["channel_id"])
            author = self.bot.get_user(record["user_id"]) or await self.bot.fetch_user(
                record["user_id"]
            )
            ...
            if len(diff_text) > 2048:
                ...
                embed.description = (
                    f"Diff is too large, so I put it on [MystB.in]({url})."
                )
            else:
                embed.description = (
                    formats.to_codeblock(diff_text, language="diff")
                    if diff_text
                    else None
                )
            fmt = f"Result {records.index(record)+1}/{len(records)}"
            author_id = getattr(author, "id", str(record["user_id"]))
            embed.set_footer(text=f"{fmt} | Author ID: {author_id}")
            embed.add_field(name="Jump to this message", value=f"[Here!]({jump})")
            embed.timestamp = datetime.datetime.fromtimestamp(record["edited_time"])
            embeds.append(embed)
        return embeds
    ...
    async def show_snipes(
        ...
        local_snipes = [
            snipe for snipe in self.snipe_deletes if snipe["channel_id"] == channel.id
        ]
    ...
    async def set_up_snipe(self, ctx):
        ...
        if not config.record:
            ...
            await ctx.send(
                "You're already enabled for snipes. Did you mean to disable it?"
            )
        self.get_snipe_config.invalidate(self, ctx.guild.id)
    ...
    async def snipe_desetup(self, ctx):
        ...
        confirm = await ctx.prompt(
            "This will delete all data stored from this guild from my snipes. Are you sure?"
        )
        if not confirm:
    ...
    @show_snipes.command(
        name="optin",
        aliases=["in", "enable"],
        usage="[member/channel] (defaults to self.)",
    )
    @requires_snipe()
    ...
    async def show_edit_snipes(
        ...
        full_results = dict_results + local_snipes
        full_results = sorted(
            full_results, key=lambda d: d["edited_time"], reverse=True
        )[:amount]
    ...
    async def _snipe_clear(
        ...
        if isinstance(target, discord.Member):
            ...
            deletes = (
                "DELETE FROM snipe_deletes WHERE guild_id = $1 and channel_id = $2;"
            )
            ...
            return
        confirm = await ctx.prompt(
            "This is a destructive action and non-recoverable. Are you sure?"
        )
        if not confirm:
            return
    ...
    async def snipe_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            return await ctx.send(
                f"Ha! Snipes are on cooldown for {error.retry_after:.02f}s."
            )
